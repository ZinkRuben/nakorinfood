...
task_port_args.add_argument("summary", type=str, help="Summary is required", required=True)
...
class ToDo(Resource):
    ...
    @marshal_with(resource_fields)
    def put(self, todo_id):  # todo change put to patch
        args = task_put_args.parse_args()
        task = ToDoModel.query.filter_by(id=todo_id).first()
        if not task:
            abort(404, message="Could not find task with that id, cannot update")
        if args['task']:
            task.task = args['task']
        if args['summary']:
